...
- Bump Rationale: Removes the project-local Delegate fleet orchestration model and redefines governance around human orchestration with direct workspace agents.
- Modified Sections:
  - Core Principle IV: Redefined the Spec Kit development lifecycle for direct workspace execution with human-chosen agents and an independent review / fix loop.
  - Core Principle V: Renamed and rewritten to "Human Orchestration & Direct Workspace Execution", eliminating the Delegate fleet, .delegate authority, and agent-to-agent delegation.
  - Quality Gates & Git Discipline: Replaced delegate references with direct workspace agents and human orchestration.
  - Governance: Updated version and amendment date, replaced delegated outputs with agent outputs.
- Added Sections: None.
...
4. Run `speckit-plan` directly in the project workspace using the agent chosen by the human operator.
5. Review the canonical plan against the specification, Figma, repository source, governance,
   functional behavior, and validation requirements before approving.
6. Run `speckit-checklist`, evaluate the checklist, run `speckit-tasks`, and run `speckit-analyze`;
   resolve every material finding before implementation.
7. Run `speckit-implement` directly in the project workspace using the agent chosen by the human operator.
8. Perform an independent review and fix loop directly in the workspace until accepted.
9. Run `speckit-converge`. If convergence adds tasks, return to the implementation and review loop
   until no required work remains.
10. Complete independent Desktop and Mobile visual validation, final technical and Git-diff
    validation, then human-only Git closure.

### V. Human Orchestration & Direct Workspace Execution

The human operator is the sole orchestrator (HUMAN = ORCHESTRATOR). The human chooses the best agent
for each Spec Kit stage and directs work directly in the project workspace. There is no
agent-to-agent delegation, no Delegate fleet, no project-local `.delegate` authority, and no
repository-defined agent routing. Agents operate directly in the workspace under human direction.
Agents MUST NOT edit canonical Spec Kit artifacts outside their designated stages or independently
alter requirements, governance, or scope. Agents MUST NOT commit, push, merge, or perform Git closure.
The human operator exclusively executes all Git commands in Warp Terminal.
...
- Pre-Commit Visual Quality Rule:
  For UI parity work, the human orchestrator MUST visually verify the implemented screen against the approved Figma frame-including relevant desktop and mobile states-before the stage is accepted for commit.
- Validation Gates:
...
- Git Workflow & Closure:
  ...
  - Agents MUST NOT execute Git closure. The human operator alone executes Commit, Push, PR,
...
This Constitution establishes non-negotiable project laws and supersedes ad-hoc preferences or unwritten assumptions. Any amendment to this document requires explicit justification, human review, and semantic version incrementing (MAJOR for principle removals/redefinitions, MINOR for new or expanded rules, PATCH for clarifications). All PRs, task plans, and agent outputs MUST be validated against this Constitution.

**Version**: 4.0.0 | **Ratified**: 2026-09-12 | **Last Amended**: 2026-09-20
